...
serviceRouter.get('/speisenart/gerichteDerKategorie/:id', function(request, response) {
    helper.log('Service Speisenart: Client requested all records');
    ...
    try {
        var result = {
            speisenart: speisenartDao.loadById(request.params.id),
            gerichte: gerichtDao.loadBySpeisenart(request.params.id)
        };
