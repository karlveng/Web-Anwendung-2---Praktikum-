...
const GerichtSpeisenartDao = require('../dao/gerichtspeisenartDao.js');
const GerichtDao = require('../dao/GerichtDao.js');
const SpeisenartDao = require('../dao/speisenartDao.js');
const express = require('express');
var serviceRouter = express.Router();

helper.log('- Service GerichtSpeisenart');

serviceRouter.get('/gerichtspeisenart/gib/:gerichtid', function(request, response) {
    helper.log('Service Gericht: Client requested one record, gerichtid=' + request.params.gerichtid);

    const gerichtspeisenartDao = new GerichtSpeisenartDao(request.app.locals.dbConnection);
    try {
        var result = gerichtspeisenartDao.loadById(request.params.gerichtid);
...
serviceRouter.get('/gerichtspeisenart/alle', function(request, response) {
    helper.log('Service Gericht: Client requested all records');

    const gerichtspeisenartDao = new GerichtSpeisenartDao(request.app.locals.dbConnection);
    try {
        var result = gerichtspeisenartDao.loadAll();
...
serviceRouter.get('/gerichtspeisenart/objekte', function(request, response) {
    helper.log('Service Gericht: Client requested all records');

    const gerichtspeisenartDao = new GerichtSpeisenartDao(request.app.locals.dbConnection);
    try {
        var result = gerichtspeisenartDao.loadAllByParent();
        helper.log('Service Gericht: Records loaded, count=' + result.length);
        response.status(200).json(helper.jsonMsgOK(result));
    } catch (ex) {
        helper.logError('Service Gericht: Error loading all records. Exception occured: ' + ex.message);
